...
- **Solo se llena lo vacío.** El servidor nunca sobrescribe `state`, `municipality` o `locality` capturados a mano, y ningún automatismo toca `status_location` (docs `adr-0024`, `adr-0027`). La excepción es de UI, no del motor: `/location/geolocate/` calcula **sin persistir** y el editor decide qué hacer con el resultado —el pin sobrescribe y avisa; el trazo solo llena vacíos— (ver la asimetría en [motor.md](references/motor.md)).
