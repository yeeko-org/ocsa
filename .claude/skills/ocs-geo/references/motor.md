...
- `api/api/views/space_time/__init__.py`, acción `/location/geolocate/`: resuelve al vuelo para el editor, sin escribir, sobre una ubicación que puede no existir aún. No pasa por `apply_geolocation` sino por los resolutores directos. Dos verbos:
  - `GET ?lat&lon&state` → `{state, municipality, locality, localities: [...]}`, vía `resolve_point`.
  - `POST {"geojson": <Feature o geometría>, "state": <id|null>}` → `{state, municipality, locality, municipalities: [{id, name, state}], localities: [...], centroid: {latitude, longitude}}`, vía `resolve_geometry`; el cuerpo, porque una línea o un polígono no cabe en la query string. `municipality` es el base (solo si el trazo cae en uno) y `state` sale del helper `_geometry_state`: el del municipio base, o el que comparten **todos** los atravesados; si discrepan, `null`, porque elegir sería capturar por el usuario.
  - `localities` es idéntica en los dos métodos y **no** es la del motor: son las candidatas a `LOCALITY_TOLERANCE_M` = 5 km o menos de lo dibujado —del punto o del trazo— dentro de los municipios que resolvió el motor, ordenadas de más cerca a más lejos y con `{id, name, municipality, distance_km, point: [lon, lat]}` cada una. `distance_km` mide contra el polígono de la localidad amanzanada (0 si lo dibujado cae dentro) y contra el punto de catálogo AGEEML cuando no hay polígono; `point` es el centroide de ese polígono o ese mismo punto de catálogo: es dónde poner la etiqueta, no contra qué se midió —una amanzanada que contiene al pin trae `distance_km` 0 y un `point` que puede quedar a kilómetros—. La lista **no se recorta en el servidor**: el front decide cuántas pinta y el aviso de localidad lejana necesita verlas todas. Sale de `localities_within`, que devuelve pares `(Locality, metros)`, más `locality_points` para el par de coordenadas. Contrato fijado por Ricardo el 2026-08-29.
...
4. Localidad (`_locality_for_point`): primero el polígono de localidad amanzanada (capa `00l`) que contenga al punto; si ninguno lo contiene, el vecino más cercano entre las localidades del municipio con `is_current=True` y coordenadas, **y solo si queda a `LOCALITY_TOLERANCE_M` = 5 km o menos**; más lejos devuelve `None`, porque en un municipio despoblado la más cercana puede estar a decenas de kilómetros y escribirla sería inventar el sitio. El índice de polígonos (`_locality_index`) también filtra `is_current`, igual que los otros dos caminos. Las dos reglas las fijó Ricardo el 2026-08-29.
...
El vecino más cercano es **solo respaldo**, y es el eslabón débil: en una mancha urbana grande que el INEGI representa con un punto único al centro, la localidad rural de al lado suele quedar más cerca que ese centro, y el resultado es una ranchería. El tope de 5 km no corrige eso —es un problema del insumo, no de la regla—; lo que corta es el otro extremo, el municipio sin ninguna localidad cerca.
...
`localities_within(geometry, municipalities, buffer_m=LOCALITY_TOLERANCE_M)` es la función que lo implementa: mide contra el polígono de la localidad amanzanada y contra el punto del catálogo AGEEML cuando no hay polígono —el mismo criterio que `far_pins` aplica al pin—, devuelve pares `(Locality, metros)` de más cerca a más lejos, y de ella sale la lista `localities` de los dos métodos del endpoint. Sirve igual a un punto que a un trazo: la distancia de shapely no distingue. Al elegir el umbral importa que las dos medidas no son equivalentes: la distancia a un punto de catálogo incluye el radio de la localidad y la distancia a su polígono no.

El marcado editorial (`space_time/review_flags.py`, comando `flag_locations_for_review`) tiene bandera propia, `--locality-threshold`, con 5 km por omisión y **sin herencia** de `--threshold` —que son los 0.5 km del pin contra su municipio—. Sus dos razones de localidad son `far_pin_locality` (el pin a más de 5 km de la localidad capturada) y `trace_off_locality` (el trazo a más de 5 km de ella, medido por distancia: **no** consulta el veredicto del motor a 500 m). `far_pin_locality` es la única razón que excluye los pines «Aprobado (Aproximado)», por la misma razón por la que el motor no les llena `locality`: su coordenada señala el rumbo y no el sitio. Contra el municipio y contra el estado sí se miden.

### Qué dicen los comentarios

**Los textos del marcado no nombran lo calculado**, solo lo capturado: los estados y municipios que el motor deriva ya se ven en la tarjeta de municipios del dashboard, y repetirlos en el comentario los congela con la fecha de la corrida. Decisión permanente de Ricardo, aplicada el 2026-08-29 a `off_state_text`, `in_other_state_text` y `far_pin_text` —que decía «El pin cae en X, no en Y» y ahora solo da la distancia y el encargo de revisar—. `in_other_state_text` sí dice **cuántos** municipios ajenos atraviesa, que es la magnitud del desborde y no un dato que la ficha repita.

### Umbrales del marcado (Ricardo, 2026-08-29)

`--threshold` bajó de 2 km a **0.5 km** (`far_pins.THRESHOLD_KM`): cualquier desfase que ya no quepa en el ruido cartográfico, y 500 m son 25 veces la simplificación de la capa municipal.

Las razones **sin umbral editorial** —las que contradicen la captura sin ambigüedad— no por eso miden con tolerancia cero: su gracia es la propia simplificación del polígono, leída de `GeometryBase.simplified_m` en la fila y nunca fijada en el código (`far_pins.state_simplification_m` / `municipality_simplification_m`). Son el pin fuera de su estado (50 m), `trace_off_municipality` (20 m, y ya no es `intersects` sino distancia) y `trace_off_state`.

Tres razones nuevas para trazos:

| Razón | Universo | Sale cuando |
|---|---|---|
| `trace_off_state` | trazo con `geojson` y `state` capturado; **no** exige municipio | el trazo se aleja del polígono estatal más que su `simplified_m` |
| `trace_in_other_state` | igual | el trazo atraviesa al menos un municipio de un estado distinto al capturado, con la regla del motor. Ve lo que `trace_off_state` no puede: un trazo puede estar a distancia cero de su estado —dentro de él, en su mayor parte— y aun así desbordar la frontera |
| `base_off_crossed` | trazo con `geojson` y `municipality` capturado | el municipio capturado no está entre los que `resolve_geometry` da por atravesados (≥ 50 m de línea o ≥ 1 ha de área) |

`trace_off_state` y `trace_in_other_state` son las únicas que alcanzan a un trazo sin municipio capturado. El caso que motivó la segunda es la ubicación 4513: un polígono repartido en trece municipios de Yucatán —el estado capturado— y uno de Campeche, a distancia cero de Yucatán y sin municipio base.

`base_off_crossed` se solapa con `trace_off_municipality` pero no lo repite: aquella pregunta si el trazo **toca** el municipio capturado y esta si lo **atraviesa**. En medio queda el roce —el trazo que entra 49 m y sigue de largo por otro municipio—, que `trace_off_municipality` da por bueno y `base_off_crossed` saca. Los atravesados se recalculan con el motor y no se leen del M2M, para comparar contra lo que el editor ve hoy.

**Asimetría aceptada como diseño** (2026-08-29, docs `adr-0026`, nota del 2026-08-29): `localities_within` solo mira localidades de los municipios que el trazo atraviesa (o del municipio resuelto, en un pin), y el marcado mide la localidad capturada sin ese filtro. No responden la misma pregunta: el aviso previsualiza lo que el motor haría; el marcado pregunta si lo capturado es plausible. Por eso el aviso del editor ya no afirma una distancia («está a más de 5 km») sino pertenencia («no está entre las cercanas»), y el front no repite la constante.
...
- **Editor, trazo** (`useGeolocate.suggestGeometry`): al dibujar, editar o importar una línea o un polígono llama al `POST` con debounce de 400 ms y **solo llena selectores vacíos** —la misma regla del servidor—, así que nunca avisa de un reemplazo. Sí avisa, sin bloquear, cuando lo ya capturado no cuadra con el trazo: el municipio capturado que no está entre los atravesados, y la localidad capturada que no viene en `localities` («no está entre las cercanas a los municipios que atraviesa el trazo»; en un pin, «al municipio del pin»). La decisión usa la lista completa que devuelve el servidor; el editor lista y pinta en el mapa solo las seis más cercanas, con nombre y kilómetros, cuando hay dos o más. Dos botones más viven ahí: el reemplazo automático (`build`/`reset_wrench`, sobrescribe con la resolución del motor y se deshace desde un snapshot que muere al redibujar o guardar) y «Guardar como comentario» en cada aviso, con la convención de `Comments.vue`. Además escribe `municipalities_full` en el registro en memoria para que `LocationMunicipalities` muestre la tira de municipios abarcados antes de guardar. Ambos caminos salen del mismo `applyFeatureAndSuggest` de `LocationEdit.vue`, que ramifica por `type_location`.
...
El paquete `space_time` corre 105 tests en total (`DATABASE_SCHEMA= python manage.py test space_time`).

El endpoint, el marcado y el editor tienen los suyos, porque su regla no es la del motor:

- `api/space_time/tests/test_geolocate_endpoint.py` (7), sobre la misma cartografía sintética (`SyntheticCartography`): el `POST` sugiere estado y municipio de un polígono contenido en uno, no sugiere estado cuando el trazo cruza dos, acepta por igual un `Feature` y una geometría pelona, devuelve en `localities` la que está a 3 km del trazo pero no la que está a 8 —ninguna de las dos lo toca, así que el caso separa la tolerancia de 5 km del contacto de 500 m— y cada candidata trae sus cinco llaves con `distance_km` y `point`. El `GET` de un punto devuelve las candidatas ordenadas por distancia (0 km la mancha urbana que lo contiene) y las deja vacías cuando la más cercana pasa de 5 km.
- `api/space_time/tests/test_review_flags.py` (29): las nueve razones del marcado editorial, incluidas `far_pin_locality` —con su exclusión de los pines aproximados y la contraprueba de que esos sí se miden contra municipio y estado—, `trace_off_locality`, `trace_off_state`, `trace_in_other_state` —con la forma del caso 4513, que ninguna otra razón ve—, `base_off_crossed` y `state_mismatch`, más la gracia por `simplified_m` de las dos razones sin umbral. `far_pin` solo corre en el caso del pin aproximado; el resto de su selección necesita la cartografía del INEGI.
